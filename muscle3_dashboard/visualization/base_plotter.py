"""The plotting half of the State/Plotter contract: reactive time-slider,
live-view toggle, and (for a ``State`` with ``auto=True``) a variable-picker
dropdown driving floating plot panels -- all domain-agnostic, since it only
ever touches ``state.data``/``state.variables``, never the messages a
``State`` was extracted from.
"""
...
from muscle3_dashboard.visualization.resizable_float_panel import (
    ResizableFloatPanel,
)
...
class BasePlotter(Viewer):
    ...
    def _update_variable_selector(self) -> None:
        ...
        assert self._state is not None
        self.variable_selector.options = sorted(list(self._state.variables.keys()))
    ...
    def _add_plot_callback(self, event: param.Event) -> None:
        ...
        plot_func = functools.partial(self._plot_variable_vs_time, full_path=full_path)
    ...
    def _floatpanel_closed_callback(
        self, full_path: str, event: param.Event | None = None
    ) -> None:
    ...
    def plot_1d(self, ds: xr.Dataset, var: Variable, time_index: int) -> hv.Element:
        ...
        coord_var = ds[f"{var.full_path}_{coord_name}"].isel(time=time_index).values
        title = f"{var.full_path} (t={float(ds.time.values[time_index]):.3f}s)"
        return hv.Curve(
            (coord_var, data_var), kdims=[coord_name], vdims=[var.full_path]
        ).opts(title=title, responsive=True)

    def plot_2d(self, ds: xr.Dataset, var: Variable, time_index: int) -> hv.Element:
    ...
    def _plot_variable_vs_time(self, full_path: str, time: float) -> hv.Element:
        ...
        if var.dimension == Dim.ZERO_D:
            ...
            v_vals = ds[var.full_path].isel(time=slice(0, time_index + 1)).values
            return hv.Curve(
